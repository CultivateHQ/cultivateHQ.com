...
Once Docker is installed, you need to create a **Dockerfile**. A Dockerfile is a set of instructions for Docker to build an image. Think of it as the blueprint or source code.
...
We're telling Docker to map our system's port 80 to the container's port 4000. If we visit http://localhost/shark.gif in our browser, we'll see a very nice image of a shark. Since these are containers, based on our image, we can even run another instance, say on port 90:
